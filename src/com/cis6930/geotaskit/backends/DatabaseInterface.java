...
public class DatabaseInterface {
  ...
  public ArrayList<Task> getTasks() {
    ...
    Cursor returnData = readDatabase.query(OpenHelper.DICTIONARY_TABLE_NAME, columnsToRead, null,
  ...
  public ArrayList<Task> getTasksWithKeyword(String keyword) {
    ...
    columnsToRead[4] = OpenHelper.KEY_LATTITUDE;
    columnsToRead[5] = OpenHelper.KEY_LONGITUDE;
    // Note that there WHERE is excluded for our purposes, just assume that it
    // is the first word in the String
    String whereClause = OpenHelper.KEY_DESCRIPTION + " LIKE %" + keyword + "% OR "
        + OpenHelper.KEY_DESCRIPTION_SHORT + " LIKE %" + keyword + "%";
    String orderBy = OpenHelper.KEY_PRIORITY;
    Cursor returnData = readDatabase.query(OpenHelper.DICTIONARY_TABLE_NAME, columnsToRead,
   ...
   * 
   * Note that this method assumes that you want ALL of the columns to be returned. If the cursor does not provide this data, then an Exception will be thrown.
   * 
   * @param returnData
   *          The Cursor representing the values returned from the SQLDatabase
  ...
  private ArrayList<Task> extractArrayList(Cursor returnData) {
    ...
    while (!returnData.isAfterLast()) {
      ...
      output.add(count,
          new Task(returnData.getInt(0), returnData.getString(1), returnData.getString(2),
              returnData.getString(3), returnData.getFloat(4), returnData.getFloat(5)));
      // Advance the Cursor
  ...
  public void addTask(Task task) {
    ...
    newValue.put(OpenHelper.KEY_LATTITUDE, task.latitude);
  ...
  public int removeTask(Task task) {
    ...
    String whereClause = OpenHelper.KEY_PRIORITY + " = \'" + task.color_priority + "\'" + " AND "
        + OpenHelper.KEY_DESCRIPTION_SHORT + " = \'" + task.name + "\'" + " AND "
        + OpenHelper.KEY_DESCRIPTION + " = \'" + task.description + "\'" + " AND "
        + OpenHelper.KEY_DISTANCE + " = \'" + task.miles_left + "\'";
